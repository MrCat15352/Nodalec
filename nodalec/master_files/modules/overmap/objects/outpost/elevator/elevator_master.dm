...
/datum/elevator_master/proc/arrive_on_floor()
	...
	if(display)
		display.set_messages("[cur_index + fake_floors + floor_offset]", seeking_dir == UP ? "UP" : "DOWN")
		display.update()
		if(open_on_floor)
			playsound(
				display,
				seeking_dir == UP ? 'nodalec/master_files/sound/machines/ding.ogg' : 'nodalec/master_files/sound/machines/ding_twice.ogg',
				100,
				FALSE
			)
		else
			// quieter
			playsound(display, 'nodalec/master_files/sound/misc/compiler-stage1.ogg', 75, FALSE)
