...
/obj/machinery/elevator_call_button
	...
	icon = 'nodalec/master_files/modules/overmap/icons/obj/stationobjs.dmi'
	icon_state = "elevator"
...
/obj/machinery/elevator_call_button/ui_interact(mob/user)
	...
	if(floor_num < length(my_floor.master.floor_list))
		...
		opts["Up"] = image(icon = 'nodalec/master_files/modules/overmap/icons/misc/arrows.dmi', icon_state = up_arrow, dir = NORTH)
	if(floor_num > 1)
		var/down_arrow = my_floor.calls & DOWN ? "green_arrow" : "red_arrow"
		opts["Down"] = image(icon = 'nodalec/master_files/modules/overmap/icons/misc/arrows.dmi', icon_state = down_arrow, dir = SOUTH)
