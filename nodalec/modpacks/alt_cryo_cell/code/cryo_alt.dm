...
/atom/movable/visual/cryo_occupant_alt
...
/atom/movable/visual/cryo_occupant_alt/Initialize(mapload, obj/machinery/cryo_cell_alt/parent)
	. = ..()
	// Alpha masking
	// It will follow this as the animation goes, but that's no problem as the "mask" icon state
	// already accounts for this.
	RegisterSignal(parent, COMSIG_MACHINERY_SET_OCCUPANT, PROC_REF(on_set_occupant_alt))
	RegisterSignal(parent, COMSIG_CRYO_SET_ON, PROC_REF(on_set_on_alt))




/// COMSIG_MACHINERY_SET_OCCUPANT callback
/atom/movable/visual/cryo_occupant_alt/proc/on_set_occupant_alt(datum/source, mob/living/new_occupant)
	SIGNAL_HANDLER
...
/atom/movable/visual/cryo_occupant_alt/proc/on_set_on_alt(datum/source, on)
	SIGNAL_HANDLER

	if(on)
		animate(src, pixel_y = 12, time = 20, loop = -1)
		animate(pixel_y = 10, time = 20)
	else
		animate(src)

/// Cryo cell
/obj/machinery/cryo_cell_alt
	name = "cryo cell"
	icon = 'nodalec/modpacks/alt_cryo_cell/icons/cryogenics_alt.dmi'
	icon_state = "pod_preview"
	density = TRUE
	max_integrity = 350
	armor_type = /datum/armor/unary_cryo_cell_alt
	...
	var/atom/movable/visual/cryo_occupant_alt/occupant_vis
	...
	var/static/mutable_appearance/cached_opening

	var/static/mutable_appearance/cached_closing

/datum/armor/unary_cryo_cell_alt
	energy = 100
	fire = 30
	acid = 30

/obj/machinery/cryo_cell_alt/Initialize(mapload)
	. = ..()

	if(!cached_opening)
		cached_opening = mutable_appearance(icon, "glass_opening")
	if(!cached_closing)
		cached_closing = mutable_appearance(icon, "glass_closing")
...
/obj/machinery/cryo_cell_alt/Destroy()
	on = FALSE
...
/obj/machinery/cryo_cell_alt/handle_deconstruct(disassembled)
...
/obj/machinery/cryo_cell_alt/on_deconstruction(disassembled)
	if(beaker)
		beaker.forceMove(drop_location())

/obj/machinery/cryo_cell_alt/contents_explosion(severity, target)
	. = ..()
	if(!beaker)
		return
...
/obj/machinery/cryo_cell_alt/Exited(atom/movable/gone, direction)
	. = ..()
	if(gone == beaker)
		beaker = null

/obj/machinery/cryo_cell_alt/add_context(atom/source, list/context, obj/item/held_item, mob/user)
...
/obj/machinery/cryo_cell_alt/examine(mob/user) //this is leaving out everything but efficiency since they follow the same idea of "better beaker, better results"
	. = ..()
...
/obj/machinery/cryo_cell_alt/update_icon()
	SET_PLANE_IMPLICIT(src, initial(plane))
	return ..()

/obj/machinery/cryo_cell_alt/update_icon_state()

	icon_state = "pod_preview"
	return ..()

/obj/machinery/cryo_cell_alt/update_overlays()
	. = ..()
	if(powered())
		var/warn_state = "off"
		if(on)
			warn_state = "safe"
			if(internal_connector.gas_connector.airs[1].temperature >= T0C)
				warn_state = "danger"
			else if(internal_connector.gas_connector.airs[1].temperature >= 170)
				warn_state = "warn"

		var/mutable_appearance/warn_state_overlay = mutable_appearance(icon, icon_state = "lights_[warn_state]", layer = ABOVE_MOB_LAYER + 0.1)
		. += warn_state_overlay
	...
	if (on && occupant)
		var/mutable_appearance/pod_over = new(src)
		pod_over.icon = 'nodalec/modpacks/alt_cryo_cell/icons/cryogenics_alt.dmi'
		pod_over.icon_state = "pod_over"
		pod_over.layer = ABOVE_MOB_LAYER
		. += pod_over

	if (on && is_operational)
		var/mutable_appearance/pod_liquid = new(src)
		pod_liquid.icon = 'nodalec/modpacks/alt_cryo_cell/icons/cryogenics_alt.dmi'
		pod_liquid.icon_state = "pod_liquid"
		pod_liquid.layer = ABOVE_MOB_LAYER
		pod_liquid.alpha = 100
		. += pod_liquid
...
/obj/machinery/cryo_cell_alt/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	. = NONE
...
/obj/machinery/cryo_cell_alt/screwdriver_act(mob/living/user, obj/item/tool)
...
/obj/machinery/cryo_cell_alt/crowbar_act(mob/living/user, obj/item/tool)
	. = ITEM_INTERACT_BLOCKING
	if(on)
		balloon_alert(user, "turn off!")
		return
...
/obj/machinery/cryo_cell_alt/wrench_act(mob/living/user, obj/item/tool)
	. = ITEM_INTERACT_BLOCKING
	if(on)
		balloon_alert(user, "turn off!")
		return
...
/obj/machinery/cryo_cell_alt/on_changed_z_level(turf/old_turf, turf/new_turf, same_z_layer, notify_contents)
	. = ..()
	if(same_z_layer)
		return
	SET_PLANE(occupant_vis, PLANE_TO_TRUE(occupant_vis.plane), new_turf)

/obj/machinery/cryo_cell_alt/set_occupant(atom/movable/new_occupant)
	. = ..()
	update_appearance()

/obj/machinery/cryo_cell_alt/RefreshParts()
	. = ..()
...
/obj/machinery/cryo_cell_alt/dump_inventory_contents(list/subset = list())
 ...
 */
/obj/machinery/cryo_cell_alt/proc/set_on(active)
	PRIVATE_PROC(TRUE)
...
/obj/machinery/cryo_cell_alt/begin_processing()
	. = ..()
	SSair.start_processing_machine(src)
	if(soundloop)
		soundloop.start()

/obj/machinery/cryo_cell_alt/end_processing()
	. = ..()
	SSair.stop_processing_machine(src)
	if(soundloop)
		soundloop.stop()

/obj/machinery/cryo_cell_alt/on_set_is_operational(old_value)
	//Turned off
	if(old_value)
		set_on(FALSE)

/obj/machinery/cryo_cell_alt/process(seconds_per_tick)
...
/obj/machinery/cryo_cell_alt/process_atmos()
	if(!on)
		return PROCESS_KILL
...
/obj/machinery/cryo_cell_alt/handle_internal_lifeform(mob/lifeform_inside_me, breath_request)
	if(breath_request <= 0)
		return null
...
/obj/machinery/cryo_cell_alt/assume_air(datum/gas_mixture/giver)
	internal_connector.gas_connector.airs[1].merge(giver)

/obj/machinery/cryo_cell_alt/return_temperature()
...
/obj/machinery/cryo_cell_alt/open_machine(drop = TRUE, density_to_set = FALSE)
	if(!state_open && !panel_open)
		set_on(FALSE)

	add_overlay(cached_opening)
	addtimer(CALLBACK(src, .proc/remove_animation_overlay, cached_opening), 13)

	return ..()

/obj/machinery/cryo_cell_alt/close_machine(mob/living/carbon/user, density_to_set = TRUE)
	treating_wounds = FALSE
	if(state_open && !panel_open)
		add_overlay(cached_closing)
		addtimer(CALLBACK(src, .proc/remove_animation_overlay, cached_closing), 13)

		. = ..()
		if(!QDELETED(occupant))
			set_on(TRUE)

/obj/machinery/cryo_cell_alt/proc/remove_animation_overlay(mutable_appearance/overlay)
	cut_overlay(overlay)

/obj/machinery/cryo_cell_alt/container_resist_act(mob/living/user)
...
/obj/machinery/cryo_cell_alt/ui_state(mob/user)
	return GLOB.notcontained_state

/obj/machinery/cryo_cell_alt/ui_interact(mob/user, datum/tgui/ui)
	ui = SStgui.try_update_ui(user, src, ui)
	if(!ui)
		ui = new(user, src, "Cryo", name)
		ui.open()

/obj/machinery/cryo_cell_alt/ui_static_data(mob/user)
	. = list()
	.["T0C"] = T0C

/obj/machinery/cryo_cell_alt/ui_data()
	. = list()
	.["isOperating"] = on
	.["isOpen"] = state_open
	.["autoEject"] = autoeject
...
/obj/machinery/cryo_cell_alt/ui_act(action, params, datum/tgui/ui, datum/ui_state/state)
	. = ..()
	if(.)
		return
...
/obj/machinery/cryo_cell_alt/can_interact(mob/user)
	//must not be in the machine or on its turf to interact
	if(get_turf(user) == get_turf(src))
		return FALSE
	return ..()

/obj/machinery/cryo_cell_alt/click_ctrl(mob/user)
...
/obj/machinery/cryo_cell_alt/click_alt(mob/user)
...
/obj/machinery/cryo_cell_alt/mouse_drop_receive(mob/target, mob/user, params)
	if(!iscarbon(target))
		return
...
/obj/machinery/cryo_cell_alt/get_remote_view_fullscreens(mob/user)
